    ...
    for item_outer in floor:
        for item_inner in floor:
            if item_outer == item_inner:
                combos.append([item_inner])
            elif item_outer.item_type == item_inner.item_type and [item_inner, item_outer] not in combos:
                combos.append([item_outer, item_inner])

    for item in floor:
        if not item.is_generator():
            if Item(item.element + ' ' + 'generator') in floor:
                combos.append([item, Item(item.element + ' generator')])
                break
    return combos
...
floors = []
states = []
with open('input2.txt', 'r') as floor_configs:
...
while not current_state.is_final_state():
    ...
    if not count % 1000:
        ...
        print 'unique states searched:', len(searched_states)
        print 'states remaining:', len(states)
        print
